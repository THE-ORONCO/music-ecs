class_name S_TimelineProgressor
extends System
...
func query() -> QueryBuilder:
	return q.with_all([C_Timeline]).iterate([C_Timeline]) # Use q.with_all([YourComponent])
...
func process(entities: Array[Entity], components: Array, delta: float) -> void:
	...
	var timelines: Array= components[0]
	for i in range(entities.size()):
		var timeline: C_Timeline = timelines[i]
		var progress_delta := timeline.tempo * delta
		timeline.progress = wrapf(progress_delta, 0., timeline.end)
